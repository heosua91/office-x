...
| ENTR-009 |
| 会議中（録音あり） |
| 会議・商談中のメイン機能画面。<br>資料のミラーリング（レーザーポインター共有）、リアクション等によるコミュニケーション、及び退室機能を提供する。<br>フローティングウィンドウの「AI対話」機能により、会議中いつでもAIへの質問が可能。<br>主催者は録音一時停止ボタン（録音自体を停止）および機密情報ボタン（録音継続・議事録から除外）を操作可能。<br>途中参加用QRコードモーダルには確認コード（数字4〜6桁）を併記し、PC等からも接続可能。 |
