...
| **Mục tiêu hiệu năng** | Tốc độ phản hồi | < **3 giây** | **Đáp ứng.**<br>- **API:** Xử lý nhanh nhờ Node.js & Redis Cache.<br>- **Media:** Đặt tại **Public Subnet** giúp giảm latency tối đa cho WebRTC. |
| | Xử lý AI (Batch) | N/A | **Lưu ý.** Xử lý Speech-to-Text & Summarize tốn thời gian (có thể > 30s). Dùng cơ chế **Queue (SQS + Worker EC2)** là hợp lý để không chặn UI của người dùng. |
...
| Hạng mục | Chi tiết | Yêu cầu (Target) | Đánh giá & Giải pháp Kỹ thuật (Từ Infrastructure_2) |
| :--- | :--- | :--- | :--- |
| **Thời gian vận hành** | Thông thường | **24/365** | **Đáp ứng.** Hệ thống thiết kế để chạy liên tục. Việc deploy code mới sử dụng **CodeDeploy (Rolling Update)** giúp không downtime (Zero Downtime Deployment). |
