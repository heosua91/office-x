...
| **Mục tiêu hiệu năng** | Tốc độ phản hồi (Response Time) | < **3 giây** | **Đáp ứng.**<br>- **API:** Xử lý nhanh nhờ Node.js & Redis Cache.<br>- **Vector Search:** Qdrant phản hồi tốc độ mili-giây cho truy vấn ngữ nghĩa.<br>- **Media:** Đặt tại **Public Subnet** giúp giảm latency tối đa cho WebRTC. |
...
| **Sao lưu (Backup)** | DB Backup | Daily | **Đáp ứng.** Cấu hình **RDS Automated Backup** chạy hàng ngày (Snapshot) và lưu Transaction Log mỗi 5 phút (Point-in-time recovery). **Qdrant** thiết lập Snapshot định kỳ lên S3. |
